...
set "JAR=%APP_DIR%..\target\java_chat_app-1.0-SNAPSHOT-jar-with-dependencies.jar"

REM Unzip JDK to run-on-windows if not already extracted
if not exist "%JDK_DIR%\bin\java.exe" (
    if exist "%JDK_ZIP%" (
        echo Extracting JDK to run-on-windows folder...
...
REM Run the GUI client in a new window
start "" cmd /k ""%JAVA_HOME%\bin\java.exe" -jar "%JAR%" GUI"
